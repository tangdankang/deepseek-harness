...
- 精确性来自封闭集合匹配，与斜杠命令完全一致：只包含一个文本 part 且以 `/` 开头的 `session.prompt` 会先对照命令注册表解析，再把匹配用户可调用 Skill 的输入作为普通提示词放行；两边都没有的名称返回 `unknown-command`。在已放行的普通提示词内，pre-step 监听器会让未知名称保持为普通行文，绝不猜测。只扫描 `source.kind === 'user'` 的消息——外部文本无法伪造手势。路径（`/usr/bin`）、分数（`5/8`）与带前缀的 token（`foo/name`）都会破坏该边界。
- 客户端沿用[纯文本引用决策](../architecture/2026-07-25-web-input-machine-and-slash-pipeline.md)：菜单 pick 落下字面文本 `/name `，该文本随提示词原样提交；ui-skill 不实现任何裁决钩子，也没有引用 codec。`skill.list`（现在是该领域唯一的 RPC）提供每一个用户可调用的 skill 并携带 `modelInvocable`，供菜单标出仅限用户的条目。它读取已附加或已持久化的会话头，并在不恢复冷 Agent 的前提下解析实时 Agent 或已记录的 preset scope，因此重连会在下一个轮次前恢复菜单。Host 根据同一个 Agent 组合完成命令与 Skill 裁决；与命令同名的名称解析为该命令。
